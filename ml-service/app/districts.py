...
#
# All three boxes are deliberately the same size (0.35 deg square, centred on
# the district town). Equal footprints matter more than exact borders here:
# if one district were sampled over a larger box it would average over more
...
#
# Sheikhupura's box was 0.75 x 0.60 before this change; it is now 0.35 square
# like the others. No satellite_features rows existed yet, so nothing was
# invalidated by the change.
DISTRICTS = {
    "Sheikhupura": (73.80, 31.54, 74.15, 31.89),  # town 31.7131 N, 73.9783 E
    "Okara":       (73.28, 30.64, 73.63, 30.99),  # town 30.8103 N, 73.4459 E
    "Sahiwal":     (72.93, 30.49, 73.28, 30.84),  # town 30.6682 N, 73.1114 E
}
...
if __name__ == "__main__":
    ...
    for name, (lng0, lat0, lng1, lat1) in DISTRICTS.items():
        ...
        assert abs((lng1 - lng0) - 0.35) < 1e-9 and abs((lat1 - lat0) - 0.35) < 1e-9, \
            f"{name}: boxes must stay equal-sized"
